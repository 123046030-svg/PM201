...
function buscar_ingrediente(nombre) {
    return inventario_cocina.find(item => item.ingrediente === nombre);
}

//cocina modulo preparar
function preparar_cafe() {
    ...
    let faltantes = receta_cafe.filter(item => {
        let ingrediente = buscar_ingrediente(item.ingrediente);
        return !ingrediente || ingrediente.cantidad < item.cantidad;
    });

    if (faltantes.length > 0) {
        console.log(`No se puede preparar el café. Falta ingrediente:`);
...
function simular_error_cocina() {
    ...
    let error = prompt(`Describe el error ocurrido en cocina: `);
...
//cocina modulo falta ingrediente
function simular_falta_ingrediente() {
    console.log(`\nSimular falta de ingrediente:`);
    ver_ingredientes();

    let nombre = prompt(`Ingrese el ingrediente que desea agotar: `).toLowerCase();

    let ingrediente = buscar_ingrediente(nombre);

    if (ingrediente) {
        ingrediente.cantidad = 0;
        console.log(`El ingrediente "${nombre}" ahora está agotado.`);
    } else {
        console.log(`El ingrediente no existe en el inventario.`);
    }
}
